...
- Admin panel: dashboard stats, product CRUD, order status management — fully separate login and database from customers (see below)
...
Koorm uses **two separate MySQL databases** — customer/catalog data and admin accounts are fully isolated (different database, different JWT secret, no shared table or login path):

```bash
mysql -u root -p < server/schema.sql        # koorm_db: users, products, orders, etc.
mysql -u root -p < server/admin_schema.sql  # koorm_admin_db: admins only
```
...
npm run seed             # creates admin@koorm.com / Admin@123 in koorm_admin_db
...
- **Admin:** separate login at `/admin/login` (also linked in the footer) — `admin@koorm.com` / `Admin@123`, created by `npm run seed`. Admin sessions are stored independently in the browser (`koorm_admin` in localStorage) from customer sessions (`koorm_user`), so you can be logged in as both at once in the same browser.

## Notes

- Checkout only supports Cash on Delivery; no payment gateway is integrated.
- Admin and customer identities are fully separate: `koorm_db.users` (no `role` column — every row is a customer) vs. `koorm_admin_db.admins`, each with its own JWT secret (`JWT_SECRET` / `ADMIN_JWT_SECRET`) and auth middleware (`protect` / `protectAdmin` in `server/middleware/auth.js`). A customer token is rejected by every `/api/admin/*` route and an admin token is rejected by every customer route.
