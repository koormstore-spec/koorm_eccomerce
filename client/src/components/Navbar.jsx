    ...
    <header className={`sticky top-0 z-50 border-b border-sand/80 bg-cream/95 backdrop-blur-xl transition-shadow ${scrolled ? 'shadow-[0_4px_18px_rgba(26,26,26,0.06)]' : ''}`}>
      ...
      <div className="container-x grid h-[4.5rem] lg:h-20 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 lg:grid-cols-[1fr_auto_1fr]">
        <div className="flex min-w-0 items-center">
          <button className="flex h-11 w-11 items-center justify-center text-ink lg:hidden" onClick={() => { setUserMenuOpen(false); setMenuOpen(true); }} aria-label="Open menu" aria-expanded={menuOpen} aria-controls="mobile-navigation">
            <MenuIcon />
          </button>
          <nav className="hidden items-center gap-5 lg:flex">
            {NAV_LINKS.map((link) => (
              <Link key={link.label} to={link.to} className="relative whitespace-nowrap text-[12px] font-medium tracking-[0.02em] text-muted transition-colors hover:text-accent link-underline">
                {link.label}
              </Link>
            ))}
          </nav>
        ...
        <Link to="/" className="group flex items-center justify-self-center gap-2 font-serif text-[1.65rem] font-bold tracking-[-0.08em] text-ink sm:text-3xl" aria-label="Koorm home">
        ...
        <div className="flex min-w-0 items-center justify-end gap-1.5 sm:gap-2">
          <form onSubmit={submitSearch} className="hidden h-10 w-36 items-center rounded-sm border border-sand bg-white/70 px-3 transition-all focus-within:border-ink xl:flex xl:w-52">
      ...
      <form onSubmit={submitSearch} className="container-x flex h-11 items-center rounded-none border-t border-sand bg-cream px-5 md:hidden">
      ...
      <Drawer open={menuOpen} onClose={() => setMenuOpen(false)} id="mobile-navigation" label="Browse the collection">
          <div className="flex h-20 shrink-0 items-center justify-between border-b border-sand px-5 sm:px-6">
            ...
            <button onClick={() => setMenuOpen(false)} aria-label="Close menu" className="btn-icon"><CloseIcon width={18} height={18} /></button>
          </div>
          ...
          <nav className="px-6 pt-3">
            ...
            {!user && <div className="mb-6 mt-4 grid grid-cols-2 gap-3"><Link to="/login" onClick={() => setMenuOpen(false)} className="btn-outline px-3 py-2.5 text-center text-[12px]">Login</Link><Link to="/register" onClick={() => setMenuOpen(false)} className="btn-primary px-3 py-2.5 text-center text-[12px]">Join Koorm</Link></div>}
          </nav>
