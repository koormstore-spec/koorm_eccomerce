    ...
    <div className="container-x py-8 fade-in md:py-12">
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] xl:gap-12">
        <section className="border-t border-ink"><div className="flex items-center justify-between border-b border-sand py-4"><p className="text-[12px] font-bold uppercase tracking-[0.16em] text-muted">{cartItems.length} item{cartItems.length === 1 ? '' : 's'}</p><p className="text-xs text-muted">Delivery across India</p></div>
          <div className="divide-y divide-sand">{cartItems.map((item) => { const price = Number(item.discount_price || item.price); const originalPrice = Number(item.price); return <article key={item.id} className="grid grid-cols-[5.5rem_minmax(0,1fr)_auto] gap-4 py-5 sm:grid-cols-[7rem_minmax(0,1fr)_auto] sm:gap-5"><Link to={`/product/${item.slug}`} className="aspect-[3/4] overflow-hidden bg-sand/40"><img src={item.images?.[0]} alt={item.name} className="h-full w-full object-cover transition-transform duration-500 hover:scale-105" /></Link><div className="flex min-w-0 flex-col"><p className="text-[12px] font-bold uppercase tracking-[0.12em] text-muted">{item.brand || 'Koorm'}</p><Link to={`/product/${item.slug}`} className="mt-1 truncate text-sm font-semibold hover:text-accent">{item.name}</Link><p className="mt-1 text-xs text-muted">Size: <span className="font-medium text-ink">{item.size}</span></p><div className="mt-auto flex items-center gap-4 pt-4"><div className="flex w-28 items-center border border-sand bg-white"><button onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} className="flex h-9 flex-1 items-center justify-center hover:bg-cream" aria-label="Decrease quantity"><MinusIcon width={13} height={13} /></button><span className="flex-1 text-center text-xs font-semibold">{item.quantity}</span><button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="flex h-9 flex-1 items-center justify-center hover:bg-cream" aria-label="Increase quantity"><PlusIcon width={13} height={13} /></button></div><span className="hidden text-xs text-muted sm:block">Qty {item.quantity}</span></div></div><div className="flex flex-col items-end justify-between"><button onClick={() => removeFromCart(item.id)} className="text-muted transition-colors hover:text-red-600" aria-label={`Remove ${item.name}`}><TrashIcon width={17} height={17} /></button><div className="text-right"><p className="text-sm font-semibold">&#8377;{(price * item.quantity).toLocaleString('en-IN')}</p>{price < originalPrice && <p className="mt-1 text-[12px] text-accent">Saved &#8377;{((originalPrice - price) * item.quantity).toLocaleString('en-IN')}</p>}</div></div></article>; })}</div>
        </section>
