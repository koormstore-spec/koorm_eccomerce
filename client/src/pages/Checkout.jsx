...
export default function Checkout() {
  ...
  const placeOrder = async (event) => {
    ...
    try { const { data } = await api.post('/orders', form); await refreshCart(); navigate(`/orders/${data.id}`, { state: { justPlaced: true } }); }
  ...
  return (
    <div className="container-x py-8 fade-in md:py-12">
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] xl:gap-12">
        ...
        <aside className="h-fit bg-ink p-6 text-cream lg:sticky lg:top-32"><div className="flex items-center justify-between border-b border-cream/15 pb-4"><h2 className="font-serif text-2xl">Your order</h2><BagIcon width={19} height={19} className="text-accent" /></div><div className="max-h-72 divide-y divide-cream/10 overflow-y-auto py-2">{cartItems.map((item) => <div key={item.id} className="flex gap-3 py-3"><img src={item.images?.[0]} alt="" className="h-14 w-11 object-cover" /><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{item.name}</p><p className="mt-1 text-[12px] text-cream/55">{item.size} &middot; Qty {item.quantity}</p></div><p className="shrink-0 text-xs">&#8377;{(Number(item.discount_price || item.price) * item.quantity).toLocaleString('en-IN')}</p></div>)}</div><div className="space-y-3 border-t border-cream/15 py-5 text-sm"><div className="flex justify-between text-cream/70"><span>Subtotal</span><span className="text-cream">&#8377;{cartTotal.toLocaleString('en-IN')}</span></div><div className="flex justify-between text-cream/70"><span>Delivery</span><span className="text-cream">{shippingFee === 0 ? 'Complimentary' : `₹${shippingFee}`}</span></div></div><div className="flex justify-between border-t border-cream/15 pt-5 text-lg font-semibold"><span>Total</span><span>&#8377;{(cartTotal + shippingFee).toLocaleString('en-IN')}</span></div><p className="mt-5 text-center text-[12px] leading-5 text-cream/55">Review your order, then place it securely with cash on delivery.</p></aside>
      </div>
    </div>
  );
}
