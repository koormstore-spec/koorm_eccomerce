...
const updateCoupon = async (req, res) => {
  try {
    ...
    if (normalizedCode !== current.code) {
      const [dupe] = await pool.query('SELECT id FROM coupons WHERE code = ? AND id != ?', [normalizedCode, req.params.id]);
      if (dupe.length > 0) {
        return res.status(400).json({ message: 'A coupon with this code already exists' });
      }
    }
    ...
    await pool.query(
      ...
      [
        ...
        min_order_amount !== undefined ? min_order_amount : current.min_order_amount,
        max_discount_amount !== undefined ? max_discount_amount : current.max_discount_amount,
        usage_limit !== undefined ? usage_limit : current.usage_limit,
        expires_at !== undefined ? expires_at : current.expires_at,
